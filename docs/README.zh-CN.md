...
| `/context` | 显示上下文窗口使用率——已用 token 与 200k 上限的对比进度条 |
| `/config` | 显示当前有效配置：模型、工作目录、会话 ID、额外参数、超时时间 |
...
**Q: 如何查看上下文窗口使用情况？**
→ 至少发一条消息后，发送 `/context`——显示最后一轮消耗的 token 数及其占 200k 上限的百分比（进度条格式）。发送 `/config` 可一次性查看当前模型、工作目录、会话 ID 和额外参数。
