...
lazycoding 解决的就是这个问题。它是一个**本地网关进程**，将 Claude Code 能力暴露给任意受支持的聊天平台。设计遵循三个原则：
...
└────────────────────────────────────────────────────────────────┘
         ▲                                         ▼
  Telegram（长轮询）                        项目文件
  飞书 / QQ / 钉钉（WebSocket 出站）        /path/to/project/
  企业微信（HTTP Webhook）
```
...
internal/
  ...
  channel/
    channel.go              Channel 接口、InboundEvent、MessageHandle、
                            KeyboardButton、NewMultiAdapter（事件汇聚 + 路由）
    telegram/
      adapter.go            Telegram 长轮询、语音/文件/图片处理、
                            内联键盘发送与回调应答、SendDocument
      renderer.go           Markdown→HTML 转换、表格渲染、
                            UTF-8 安全的 Split / Truncate
    feishu/
      adapter.go            飞书 WebSocket/Webhook、互动卡片发送/更新、
                            Token 管理、AES 事件解密、语音/文件/图片下载
      renderer.go           Telegram HTML → Lark Markdown 转换、SplitText
      ws.go                 飞书 WebSocket 长连接协议（手写 protobuf 编解码）
    qqbot/
      adapter.go            QQ 群机器人 WebSocket（出站）、JSON 操作码协议、
                            bots.qq.com Token、延迟发送 Handle
    dingtalk/
      adapter.go            钉钉 Stream WebSocket（出站）、EVENT/SYSTEM 帧、
                            sessionWebhook 回复、延迟发送 Handle
    wework/
      adapter.go            企业微信 HTTP Webhook、AES-256-CBC 解密、
                            SHA1 签名验证、REST API 回复、延迟发送 Handle
...
| 字段 | 含义 |
|------|------|
| `UserKey` | `"tg:{userID}"` / `"fs:{openID}"` / `"qq:{memberOpenID}"` / `"dt:{staffID}"` / `"ww:{userID}"` — 发送者身份 |
...
| 维度 | Telegram | 飞书（默认） | 飞书（Webhook） | QQ 群机器人 | 钉钉 | 企业微信 |
|------|----------|------------|---------------|-----------|------|---------|
| 事件推送 | 长轮询 | WebSocket（出站） | HTTP Webhook | WebSocket（出站） | WebSocket（出站） | HTTP Webhook |
| 公网 IP | ❌ 不需要 | ❌ 不需要 | ✅ 需要 | ❌ 不需要 | ❌ 不需要 | ✅ 需要 |
| 消息格式 | Telegram HTML | Lark Markdown 卡片 | Lark Markdown 卡片 | 纯文本 | Markdown | Markdown |
| 编辑消息 | `editMessageText` | PATCH 卡片 | PATCH 卡片 | ❌ 延迟发送 | ❌ 延迟发送 | ❌ 延迟发送 |
| Token | 静态 Bot Token | `tenant_access_token`（2h TTL） | 同左 | `access_token`（bots.qq.com） | `accessToken`（2h TTL） | `access_token`（2h TTL） |
| 语音/文件/图片 | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| 内联键盘 | ✅ | ✅（卡片 action） | ✅ | ❌ | ❌ | ❌ |

### 延迟发送 Handle 模式

QQ、钉钉、企业微信均不支持编辑已发送的消息。这三个平台使用**延迟发送 Handle**：

1. `SendText/SendKeyboard` → 立即发送「思考中…」提示，返回 Handle
2. `UpdateText(handle, text)` → 将文本缓冲到 `handle.pending`
3. `Seal()` → 发送 `handle.pending` 中的最终完整内容

### Telegram

通过 `telegram-bot-api` 长轮询。每条 update 独立 goroutine 处理下载/转录。`tgHandle` 的 `hasKeyboard` 标志确保 `UpdateText` 首次调用时自动移除内联按钮。

### 飞书 / Lark

**WebSocket 模式（默认）：** 调用 `POST /callback/ws/endpoint` 获取 `wss://` URL，用 `gorilla/websocket` 主动拨号。帧格式为 protobuf 二进制（手写编解码器，无 SDK 依赖）。`method=0` = ping/pong，`method=1` = 事件。每个事件帧立即 ACK（`{"code":200}`）。断线后指数退避重连（2s → 60s）。

**Webhook 模式：** 启动 HTTP 服务器，飞书主动推事件。支持可选 AES-CBC-256 事件解密（`sha256(encrypt_key)` 为 AES key）。

**卡片格式：** 互动卡片含 `lark_md` div + 可选 `action` 按钮。`UpdateText` 通过 PATCH 原地更新卡片（与 Telegram `editMessageText` 效果一致）。

**渲染器：** `TelegramHTMLToLarkMarkdown` → Lark Markdown；`SplitText` 在换行边界分割（上限 3000 字符）。

### QQ 群机器人

**协议：** 出站 WebSocket 至 `wss://api.sgroup.qq.com/websocket`。JSON 操作码握手：OP10 Hello → OP2 Identify（intent=`GROUP_AND_C2C_EVENT`，1<<25）→ OP11 心跳 ACK 循环。OP0 Dispatch 携带 `GROUP_AT_MESSAGE_CREATE` 事件。

**Token：** POST `https://bots.qq.com/app/getAppAccessToken`，Authorization 头：`QQBot <token>`。

**回复：** `POST /v2/groups/{group_openid}/messages`，携带 `msg_id`（原始用户消息 ID，5 分钟被动回复窗口）。Adapter 维护 `group_openid → msg_id` 映射。

**渲染器：** `htmlToPlainText`（剥离所有 HTML 标签，反转义实体）。QQ 仅支持纯文本。

### 钉钉 Stream

**协议：** 调用 `POST https://api.dingtalk.com/v1.0/gateway/connections/open`（携带 `x-acs-dingtalk-access-token`）获取 WebSocket endpoint URL，直接连接。帧为 JSON，`type` 字段区分 SYSTEM（ping/pong）和 EVENT（bot 消息）。EVENT 帧 ACK 后异步处理。

**回复：** 每条消息携带 `sessionWebhook` URL（有效约 2 小时），POST 至该 URL 即可回复（无需 Token）。Body：`{msgtype:"markdown", markdown:{title, text}}`。

**延迟发送：** `Seal()` 向最新存储的 `sessionWebhook` 发送最终内容。

**渲染器：** `htmlToMarkdown`（同飞书 `TelegramHTMLToLarkMarkdown` 逻辑）。

### 企业微信

**协议：** HTTP Webhook 服务器。GET = URL 验证（解密 `echostr` 并返回明文）。POST = 消息回调（解密 XML，派发）。

**解密：** `EncodingAESKey`（43 个 base64 字符 + "="）→ 32 字节 AES Key。IV = Key 前 16 字节。AES-256-CBC 解密 → PKCS7 去填充 → 跳过 16 字节随机前缀 + 4 字节长度字段 → XML 内容 + corpID 后缀。

**签名：** `SHA1(sorted([token, timestamp, nonce, encrypt]))`。

**回复：** GET `/cgi-bin/gettoken` 获取 access token，POST `/cgi-bin/message/send`，格式：`{touser, msgtype:"markdown", agentid, markdown:{content}}`。

**渲染器：** `htmlToMarkdown`（同钉钉）。
...
**内置 adapter：**
- **Telegram** (`internal/channel/telegram`) — 长轮询、语音、文件上传/下载、内联键盘、原地编辑流式输出
- **飞书/Lark** (`internal/channel/feishu`) — WebSocket + Webhook、互动卡片、AES 事件解密、语音/文件/图片、原地编辑
- **QQ 群机器人** (`internal/channel/qqbot`) — 出站 WebSocket、JSON 操作码、延迟发送 Handle
- **钉钉** (`internal/channel/dingtalk`) — Stream WebSocket、sessionWebhook 回复、延迟发送 Handle
- **企业微信** (`internal/channel/wework`) — HTTP Webhook、AES-CBC 解密、REST API 回复、延迟发送 Handle

**`cmd/lazycoding/main.go` 中的 adapter 初始化——所有平台可同时启用：**
```go
